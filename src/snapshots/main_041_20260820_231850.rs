...
fn main() {
    ...
    if let Some(num) = raphael.get("Blue") {
        println!("value of Blue is: {}", num);
    } else {
        eprintln!("There is no number ...");
    }

    let Some(value) = raphael.get("Red") else {
        panic!("There is no number for Red ...");
    };
