...
// Goal: Initialize vector with enum
...
fn main() {
    ...
    let cell_vec: Vec<SpreadSheetCell> = vec![
        SpreadSheetCell::Int(120),
        SpreadSheetCell::Float(32.36),
        SpreadSheetCell::Text(String::from("I am beginner Rustacean ...")),
    ];

    for item in &cell_vec {
        match item {
            SpreadSheetCell::Int(num) => println!("value of integer is: {}", num),
            SpreadSheetCell::Float(f_num) => println!("value of float is: {}", f_num),
            SpreadSheetCell::Text(txt) => println!("message is: {}", txt),
        }
    }
